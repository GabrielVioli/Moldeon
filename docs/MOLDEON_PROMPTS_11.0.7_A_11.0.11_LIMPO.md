# MOLDEON — PROMPTS 11.0.7 A 11.0.11

## HEAD GLOBAL

Fluxo canônico:

`MODELAR 2D → MONTAR 3D → COSTURAR 2D↔3D → PROVAR 3D`

Considere como contratos já estabelecidos todas as etapas anteriores manualmente aceitas. Não reimplemente nem substitua mecanismos já validados sem uma regressão concreta que exija isso.

`PatternDocumentV3` continua sendo a fonte canônica. `PatternDefinitionV3` representa a geometria técnica 2D; `PanelInstanceV3` representa a instância física; `arrangementAnchor` representa montagem/placement persistido; `SeamGroupV3` representa sewing. Não crie modelos paralelos para essas responsabilidades.

`includedIn3D` controla existência visual no workspace 3D; `simulationEnabled` controla participação na simulação. Painéis não devem desaparecer por ausência de seam ou arrangement. Quando ainda não posicionados, permanecem em staging.

O runtime canônico deve executar intenção explícita do usuário. Não determinar placement, garment role, frente/costas/lado ou comportamento físico por nome, template ID, semanticRole ou aparência geométrica. Não usar autoscale para fitting.

Modelar, Montar e Costurar operam com cloth physics OFF. Provar é a única fronteira que ativa XPBD, gravidade e colisões. Ajustar ao corpo é uma operação geométrica, não uma simulação escondida.

A escala permanece absoluta: 100 mm no 2D = 0.100 m no 3D, com `scale=1` no caminho canônico. Alterações de geometria 2D devem preservar a identidade e o arrangement das PanelInstances sempre que semanticamente válido.

Preserve os fast paths e a invalidação granular estabelecidos nas etapas anteriores. Interações como seleção, drag, rotate, flip, hover e drafts não devem provocar full assembly, remesh global, XPBD init, body rebuild ou outros trabalhos não relacionados.

Performance, memória, lifecycle, desktop e mobile são requisitos de correção. Não introduza regressões em etapas anteriores para cumprir a etapa atual.

Siga `AGENTS_RULES.md` durante toda a execução.

Parta exclusivamente do HEAD FINAL e manualmente aceito da etapa anterior. Execute somente a etapa especificada no prompt atual. Não antecipe nem inicie a etapa seguinte.

> Este arquivo é o mestre de referência. Na execução, envie ao agente apenas o HEAD global + o prompt da etapa atual.

---

# PROMPT 11.0.7 - Canonical 3D Arrangement Workspace + Direct Manipulation + Continuous Body Surface
...
Durante a geração deste prompt, a branch auditada estava em `144b9208503a88ef76b9807ef57b8eb783d4b32b`, mas esse SHA é apenas evidência da auditoria. Se a 11.0.6 tiver sido legitimamente finalizada em outro SHA, use o HEAD aceito mais recente e registre-o no relatório.

Antes de editar:

1. confirme branch/base/HEAD remoto e local;
2. confirme worktree;
3. crie `recovery/11.0.7-canonical-3d-arrangement-workspace` a partir da base aceita;
4. não modifique a branch 11.0.6;
5. não inicie 11.0.8.
...
1. `BodyReference2D.tsx` usa anchors projetados discretos e nearest-anchor/radius para confirmar placement. Isso é útil como atalho, mas quantizado demais para ser a autoridade principal.
2. `GarmentAssembly.buildGarmentAssembly()` omite peças quando `resolvePiecePlacements()` não produz placement.
3. `SemanticAvatarArrangement.buildSemanticAvatarArrangement()` também omite instâncias quando `resolveAvatarAnchor()` não encontra anchor.
4. `ResolvedAssemblyInput` filtra `includedIn3D && simulationEnabled`, o que mistura existência visual com participação na simulação.
5. `App.tsx` deriva `buildResolvedAssemblyInput(garment)` do garment inteiro e `GarmentViewport` chama `ThreeViewport.updateGarment()` quando a signature muda.
6. `ThreeViewport.updateGarment()` executa trabalho pesado: avatar, assembly worker, meshes, body registration, exact body packing e XPBD initialization. Esse caminho não pode ser o hot path de drag.
7. `HumanBodyModel` possui `visualMesh`, `surfaceRegions`, landmarks e topologia estável. Portanto é possível fazer placement contínuo na superfície sem criar outro modelo corporal.
8. O cache atual de `HumanBodyModel` é um `Map` sem limite aparente. Audite crescimento real e limite/evite retenção se confirmado.
...
O usuário mostra ao sistema onde a peça pertence. O runtime não precisa descobrir que um polígono "parece frente de camiseta" ou "parece perna".
...
É permitido estender `PanelArrangementAnchorV3` somente com a informação mínima necessária para persistir um ponto contínuo da superfície corporal.
...
Como o `HumanBodyModel` mantém topologia canônica estável entre medidas, triangle+barycentric deve ser reavaliado na malha atual quando o corpo muda, em vez de persistir uma posição mundial obsoleta.
...
Mas o usuário deve conseguir soltar uma peça em qualquer ponto válido da `HumanBodyModel.visualMesh`, inclusive entre anchors nomeados.
...
Deve ser undoable. Persistência entre sessões não é obrigatória nesta etapa se não houver um local canônico apropriado; não crie schema de domínio apenas por isso.
...
Se necessário, crie um acelerador de query de apresentação/arrangement, cacheado por body/topology signature. Isso é infraestrutura de consulta, não fonte de verdade de placement.
...
Reutilize geometria isométrica/coarse-fine já existente quando fizer sentido, mas somente em uma operação one-shot ou worker cancelável.
...
1. `includedIn3D=true`, `simulationEnabled=false`, sem seam e sem arrangement → instância existe no workspace 3D.
2. `includedIn3D=false` → não existe no workspace 3D.
3. instância sem arrangement permanece `unclassified`; staging não persiste fake anchor.
4. surface attachment roundtrip save/load.
5. mudança de medidas reavalia triangle+barycentric no mesmo topology signature.
6. rename invariance.
7. 100 mm = 0.100 m.
8. `scale !== 1` no canonical new path deve ser rejeitado/normalizado conforme política existente, nunca usado para fitting.

### 3D arrangement

9. 2 painéis sem seams aparecem simultaneamente.
10. frente/costas de camiseta aparecem simultaneamente no staging e podem ser arrastadas para lados opostos manualmente.
11. 4 painéis de calça aparecem simultaneamente e podem ser posicionados em front-L/front-R/back-L/back-R sem nomes significativos.
12. qualquer ponto válido da superfície aceita drop.
13. flip preserva geometry/metric.
14. multi-select preserva IDs.
15. pin impede drag e é undoable.
16. editar 2D após placement não apaga arrangement.

### Performance

17. 5 s de drag de um painel: `AssemblyWorkerClient.solve` count = 0 durante a gesture.
18. `buildXpbdInitialization` count = 0 durante `Montar`.
19. body packing count = 0 durante drag.
20. geometry/remesh count = 0 em transform-only drag.
21. um drag gera uma entrada de undo.
22. desktop >= 30 FPS sustentados no gate e target 60.
23. mobile >= 30 FPS no gate representativo.

### Memory

24. 30 ciclos abrir/fechar `Montar` sem crescimento monotônico significativo de heap/GPU resources.
25. 30 mudanças de medidas não deixam 30 HumanBodyModels grandes retidos se não forem necessários.
26. workers antigos terminados.
27. canvas/WebGL context não acumula.
...
1. pare novas investigações;
2. preserve o melhor estado funcional;
3. `git diff --check`;
4. rode focused tests possíveis;
5. faça commit WIP/checkpoint coerente;
6. push na branch 11.0.7;
7. registre HEAD, arquivos, testes, métricas e próximo passo.

Se o ambiente cloud só consegue executar via GitHub Actions, use no máximo um workflow temporário reutilizável. Não criar carrossel de `.yml` por experimento.

Relatório final deve trazer causa raiz, antes/depois de performance, heap soak, arquivos alterados e blockers.

PARE PARA VALIDAÇÃO MANUAL.
NÃO INICIE 11.0.8.


---

# PROMPT 11.0.8 - Bidirectional Sewing Authoring 2D↔3D + Incremental Conform of Sewn Components
...
Uma seam é semanticamente entre ranges de definitions. Sua realização física deve apontar explicitamente para as `PanelInstanceV3` relevantes através do modelo existente de bindings.
...
Reutilize `seamDraft`, `addSeamDraftRange`, `finishSeamDraftSide`, `reviewSeamDraft` e infraestrutura existente quando ainda forem válidos.
...
Após uma seam ser confirmada, o Moldeon pode ajustar geometricamente somente o componente afetado para uma pose de montagem legível.
...
Se o solve ultrapassar orçamento de um frame, executar assíncrono/cancelável e mostrar feedback curto (`Ajustando...`) sem bloquear camera/selection.
...
Se o componente pequeno típico não puder ser ajustado de forma responsiva, auto-adjust deve cair para execução explícita em vez de congelar a tela.
...
1. criar seam 1:1 inteiramente no 2D;
2. criar seam 1:1 inteiramente no 3D;
3. Side A no 3D + Side B no 2D;
4. 1:N;
5. N:M;
6. direction/reverse;
7. comprimento/delta correto;
8. save/reload;
9. editar/remover seam;
10. physicalBindings apontam para instâncias corretas;
11. 2 painéis sem seam permanecem visíveis;
12. criar seam não altera visible instance count;
13. remover seam não altera visible instance count;
14. staged panel costurado não ganha body arrangement automaticamente;
15. rename invariance;
16. no name/template inference.
...
Checkpoint/timeout segue as regras da 11.0.7: WIP commit/push somente para não perder trabalho; sem commit FINAL antes de validação manual.
...
# PROMPT 11.0.9 - Pattern Construction Semantics + Stable 3D Arrangement Across 2D Geometry Edits
...
A bancada 2D vira definitivamente a área de construção do molde. A montagem 3D permanece estável enquanto a geometria evolui.
...
Se topology permanecer compatível durante drag, uma atualização de buffer leve é permitida. Se não, espere o commit da gesture.
...
Quando a intenção pode ser persistida explicitamente, runtime canônico não deve escolher a fold edge por "maior borda reta periférica".
...
Drags não podem guardar centenas de snapshots completos. Se a history não tiver bound/coalescing adequado, implementar política mensurável sem perder undo funcional.
...
Esta etapa não deve criar um novo HumanBodyModel para cada edição de molde. Body signature só muda quando medidas/body mudam.
...
1. posicionar painel no peito → mover ponto no 2D → placement continua no peito.
2. posicionar painel atrás → alterar gola → continua atrás.
3. quatro painéis de calça montados → editar um front panel → outros três não se movem.
4. save/reload após geometry edit mantém arrangements.
5. duplicate/mirror mantém IDs/relations coerentes.

### Dart

6. ambos entry points produzem mesma semântica.
7. save/reload.
8. rename invariance.
9. bust dart gera shape coerente no Provar sem mover anchor.

### Fold

10. fold reference determinística.
11. cut-on-fold não cria falsa seam central.
12. geometry edit mantém fold reference ou invalida explicitamente se edge deixou de existir.

### Grainline

13. 0/45/90 material frame.
14. anisotropic response.
15. isotropic invariance.

### Performance

16. point drag 5 s: full assembly solve count = 0 durante pointermove.
17. body rebuild count = 0.
18. XPBD init count = 0.
19. no unrelated pattern remesh.
20. geometry commit atualiza somente definition/instances afetadas.
21. desktop/mobile editor FPS gate.
22. 50 edit/undo cycles sem heap growth monotônico.
...
# PROMPT 11.0.10 - Generic Complex Garments via Manual 3D Assembly + Generator Migration
...
Geradores especializados podem continuar existindo como ferramentas de authoring. Depois que o documento canônico existe, o runtime não deve precisar descobrir novamente "isso é manga/calça/cós" para funcionar.

## PRINCÍPIO DE PRODUTO

Um usuário deve conseguir:

1. desenhar quatro painéis sem nomes significativos;
2. entrar em Montar;
3. posicionar manualmente cada um em front-L/front-R/back-L/back-R das pernas;
4. costurar ranges explicitamente;
5. ajustar ao corpo;
6. Provar;
...
Resultado deve ser determinado pela montagem do usuário e pelas seams, não por `trouserLogicalAssembly` no runtime canônico.
...
Não implementar cloth-on-cloth/layers completos, mas remover qualquer hipótese de que somente um panel pode ocupar uma região do corpo.
...
Se houver regressão causada por esta branch, corrigir. Débito herdado de collision deve ser documentado separadamente, não mascarado.
...
1. randomize names;
2. remover template IDs não necessários;
3. provar que explicit arrangement + seams são suficientes.
...
# PROMPT 11.0.11 - Explicit Runtime Cleanup + Legacy Heuristic Removal + Browser Performance & Memory Hardening
...
| Mudança | Rebuild body | Remesh | Assembly solve | XPBD init |
|---|---:|---:|---:|---:|
| drag arrangement | não | não | não | não |
| rotate/flip arrangement | não | não | não | não |
| select/highlight | não | não | não | não |
| seam hover/draft | não | não | não | não |
| seam commit | não | não, salvo topology change | component only se ajustar | não |
| 2D geometry commit | não | definition afetada | component/local | não |
| measurement change | sim | não | re-evaluate anchors/conform | não durante edit |
| entrar Provar | cache/reuse | reuse | final STEP-0 necessário | sim |
...
Migration pode usar nomes/templates somente se isso fizer parte explicitamente do formato legado e o resultado for materializado uma vez. O runtime canônico após migration não consulta esses nomes.
...
1. profiling provando hotspot;
2. A/B correctness;
3. exact contact regression suite;
4. nenhuma redução silenciosa de precisão para "ganhar FPS".

Se a otimização física profunda ainda for grande demais, registrar debt para a etapa 12, mas eliminar todo desperdício arquitetural ao redor dela agora.
...
3. Montar;
4. mover 4 panels;
5. criar/remover seam;
6. Ajustar ao corpo;
7. Provar por alguns steps;
8. reset;
9. mudar medidas;
10. voltar Modelar;
11. unload/reload.
...
Não exigir 60 FPS do solver complexo nesta etapa se a física herdada não alcançar, mas não aceitar regressão e não aceitar que UI/render bloqueie esperando physics.
...
1. tabela REMOVE / MIGRATION / COMPATIBILITY / KEEP;
2. heurísticas removidas;
3. generic geometry preservada;
4. invalidation/performance architecture final;
5. before/after profiles;
6. memory soak charts/tables;
7. garment matrix;
8. browser/device matrix;
9. blockers herdados;
10. debt explicitamente deixado para etapa 12, se houver;
11. confirmação de que nenhuma precisão física foi relaxada silenciosamente.

Não faça commit FINAL/push antes da validação manual. Checkpoint WIP é permitido apenas para preservar trabalho em cutoff.

PARE PARA VALIDAÇÃO MANUAL.
NÃO INICIE A ETAPA 12.
...
1. Finalize e aceite manualmente a 11.0.6 atual.
2. Execute somente a 11.0.7.
3. Faça gate manual real, principalmente mobile e manipulação 3D.
4. Só então use a 11.0.8.
5. Repita a disciplina até 11.0.11.
6. Não deixe um prompt "compensar" o anterior. Regressão herdada deve ser corrigida na etapa que a introduziu ou formalmente isolada por A/B.
7. Performance/memória são gates em todas as etapas, não tarefas para o final.
8. Se uma mudança simples de UI começar a alterar `physics/**`, parar e provar a necessidade antes de continuar.
...
Sem nomear manualmente "frente", "costas", "manga" ou "perna" para o solver entender. Sem painel desaparecer por não ter seam. Sem scale escondido. Sem travar o navegador a cada drag. Sem vazamento progressivo de memória ao trabalhar por uma sessão longa.
